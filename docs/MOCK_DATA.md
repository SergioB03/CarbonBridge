...
The POC is **mostly real data now.** For the **8 core facilities**, owners, LEIs, locations, production routes, multi-year emissions intensity, and the CBAM-scope-vs-full-footprint split are a **real static extract from Climate TRACE** (`src/data/history.json`, manufacturing v5.7.0, co2e_100yr, CC BY 4.0). Only the **self-reported supplier claims** are illustrative (no public registry of self-reports exists), and the **CBAM default values / cert price** are anchored to published numbers where available. This file is the **"where would the rest of the data come from?"** reference.

**The 4 Americas comparators (`illustrative: true`, ids `9000000x`).** To stop the book skewing entirely to Asia/MENA, four US/Brazil producers were added — **Nucor**, **Alcoa**, **Gerdau**, **Albras**. They carry a **real plant, owner, location and verified LEI** (the LEIs were pulled live from the GLEIF API used elsewhere in the app), but their **emissions series is calibrated** to published route + regional-grid figures, **not** drawn from the Climate TRACE extract. They are flagged `illustrative` in the data and rendered with an **"est."** badge in the UI, and the Evidence copy says so. The standout pairing: **Alcoa's ~12 vs Albras's ~4 full cradle-to-gate footprint** at the same ~2.5 CBAM-priced intensity — US coal grid vs Brazilian hydro, the out-of-scope electricity story made concrete. To make these *fully* real, re-extract them from the Climate TRACE bulk package the same way as the core 8.

**Why a static extract, not a live API:** Climate TRACE's beta API only exposes the latest year per asset and warns it is unreliable for production. The multi-year series lives in the bulk packages, so we pre-extracted the 8 core facilities (2021–2025 + partial 2026) once — exactly the "pre-cache the CSV" guidance. The pull is reproducible from `cttmp/` (discover → download manufacturing package → parse).
